...
class PasswordPoliciesSerializer(serializers.Serializer):
    ...
    new_password_validators = [
        ...
        fields.MaxLengthValidator(
            password_settings.PASSWORD_MAX_LENGTH or 1000,
            message=lazy(
                serializers.CharField.default_error_messages["max_length"].format, str
            )(max_length=password_settings.PASSWORD_MAX_LENGTH or 1000),
        ),
        fields.MinLengthValidator(
            password_settings.PASSWORD_MIN_LENGTH or 0,
            message=lazy(
                serializers.CharField.default_error_messages["min_length"].format, str
    ...
    def validate(self, validated_data):
        ...
        try:
            ...
            self._lazy_errors.extend(fields.get_error_detail(ex))
    ...
    def run_validation(self, data=fields.empty):
        ...
        if getattr(self, "_lazy_errors", []):
            raise ValidationError(
                detail=as_serializer_error(ValidationError(self._lazy_errors))
            )
        return value
