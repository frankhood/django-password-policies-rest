...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-password-policies-rest runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-password-policies-rest.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ password_policies_rest")
...
def lint(c):
    ...
    """
    c.run("flake8 django-password-policies-rest tests")
...
def release(c, bumpsize=''):
    ...
    import django_password_policies_rest
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_password_policies_rest.__version__))
    c.run("git push --tags")
